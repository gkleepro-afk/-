      ...
      <header className="bg-white border-b border-slate-200 px-8 py-6 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-3">
            <h2 className="text-2xl font-bold text-slate-900 tracking-tight">{t('analyticsTitle')}</h2>
      ...
      <div className="p-8 max-w-7xl mx-auto w-full space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-xs flex flex-col justify-between">
            ...
            <p className="text-3xl font-black text-slate-900">{stats.streakDays} {uiLang === 'en' ? 'Days' : '天'}</p>
            <p className="text-xs text-slate-400 mt-2">{uiLang === 'en' ? 'Keep up active recall' : '打卡不断，记忆犹新'}</p>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-xs flex flex-col justify-between">
            ...
            <p className="text-3xl font-black text-slate-900">{stats.totalStudyMinutes} {uiLang === 'en' ? 'mins' : '分钟'}</p>
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-xs flex flex-col justify-between">
            ...
            <p className="text-3xl font-black text-slate-900">{stats.completedTasksCount} {uiLang === 'en' ? 'tasks' : '项'}</p>
            <p className="text-xs text-slate-400 mt-2">
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-xs flex flex-col justify-between">
            ...
            <p className="text-3xl font-black text-slate-900">{stats.averageQuizScore}%</p>
            <p className="text-xs text-slate-400 mt-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7 bg-white p-6 rounded-xl border border-slate-200 shadow-xs space-y-6">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="text-base font-bold text-slate-900">各学科掌握度 (Subject Mastery)</h3>
              <span className="text-xs text-slate-400">动态分析</span>
            </div>
              ...
              {subjectList.map(([subName, percent]) => (
                <div key={subName} className="space-y-1.5">
                  <div className="flex justify-between items-center text-xs font-bold">
                    <span className="text-slate-800">{subName}</span>
                    <span className="text-blue-600">{percent}% 掌握度</span>
                  </div>
          ...
          <div className="lg:col-span-5 bg-slate-900 text-white p-7 rounded-2xl shadow-lg flex flex-col justify-between">
